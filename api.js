...
export default async function handler(req, context) {
  ...
  if (action === "createPost") {
    ...
    // ── Facebook पर auto-post ──────────────────────────────────────
    // यह async है और साइट की response को block नहीं करता।
    // Facebook post fail होने पर भी वाक ऋचा की post सफल रहती है।
    const siteUrl = process.env.URL || process.env.DEPLOY_URL || "";
    postToFacebook({
      title: post.title,
      ...
      if (fbResult && fbResult.success) {
        console.log(`[Facebook] "${post.title}" → FB Post ID: ${fbResult.fbPostId}`);
      } else if (fbResult && fbResult.skipped) {
        console.log("[Facebook] Skip (env vars नहीं मिले)।");
      } else {
        console.warn("[Facebook] पोस्ट नहीं हो सकी:", fbResult && fbResult.error);
      }
    }).catch((e) => console.error("[Facebook] Exception:", e));
    // ─────────────────────────────────────────────────────────────
